...
-- 个人设置
(24, '个人设置', 'Setter', '/setter', 1, 1, '130bdeec588552954b9e3bea0ef364b2', 'layout', FALSE, '/setter/profile', 'c-setter', TRUE, FALSE, TRUE, 10000, 'admin', FALSE, FALSE),
(25, '个人中心', 'SetterProfile', 'profile', 24, 2, 'cce99c598cfdb9773ab041d54c3d973a', 'setterProfile', FALSE, '', 'i_user', TRUE, FALSE, TRUE, 10001, 'admin', FALSE, TRUE),
(26, '系统向导', 'SetterGuide', 'guide', 24, 2, '6602bbeb2956c035fb4cb5e844a4861b', 'setterGuide', FALSE, '', 'guide', TRUE, FALSE, TRUE, 10002, 'admin', FALSE, TRUE);
------------------------------------------------------------------------------------------------
...
-- insert sql
SELECT
  'delete from enum; insert into enum(`name`, `md5_id`, `key`, `value`, `description`, `status`, `create_rtx`, `order_id`) VALUES'
FROM sysuser
WHERE rtx_id = 'admin'
UNION ALL
SELECT
...
CREATE TABLE `sysuser_avatar`  (
  ...
  `type` varchar(35) NULL COMMENT '类型',
  `summary` varchar(200) NULL COMMENT '简要',
  `label` varchar(55) NULL COMMENT '标签，用英文;分割',
  `url` varchar(120) NOT NULL COMMENT '存储地址',
  `or_url` varchar(120) NOT NULL COMMENT '原始图片存储地址，用于图片裁剪覆盖图片信息保留',
...
CREATE TABLE `sms`  (
  ...
  `rtx_id` varchar(25) NOT NULL COMMENT '创建人RTX-ID',
  `md5_id` varchar(55) NOT NULL COMMENT '唯一标识：MD5-ID',
  `title` varchar(55) NULL COMMENT '短信标题',
  `content` varchar(255) NULL COMMENT '短信内容',
  `user` varchar(2000) NULL COMMENT '电话号码列表',

...
from deploy.config import QYWX_BASE_URL, QYWX_SEND_MESSAGE, \
    QYWX_ACCESS_TOKEN, QYWX_SEND_BACK, QYWX_TEMP_UPLOAD, QYWX_TEMP_GET, DEBUG
...
class QYWXLib(object):
    ...
    def __init__(self, corp_id, secret, agent_id):
        ...
        """
        if DEBUG:
            LOG.debug("QYWXLib class start initialize.")
    ...
    def send(self, to_user=[], to_party=[], to_tag=[],
        ...
        if not self.token:
            return Status(
                903, StatusEnum.FAILURE.value, '[企业微信]TOKEN初始化失败', {}).json()
        if not isinstance(to_user, list) \
                or not isinstance(to_party, list) \
                or not isinstance(to_tag, list):
            return Status(
                402, StatusEnum.FAILURE.value, '[企业微信]接收用户/部门参数必须为List类型', {}).json()
        if not any([to_user, to_party, to_tag]):
            return Status(
                400, StatusEnum.FAILURE.value, '[企业微信]缺少接收用户/部门列表', {}).json()
        if not content:
            return Status(
                400, StatusEnum.FAILURE.value, '[企业微信]缺少消息内容', {}).json()
        if not isinstance(content, dict):
            return Status(
                402, StatusEnum.FAILURE.value, '[企业微信]消息体参数必须为Dict类型', {}).json()
        if stype not in self.types:
            return Status(
                404, StatusEnum.FAILURE.value, '[企业微信]消息类型错误', {}).json()
        """ ================== 默认参数 ================== """
        ...
        if kwargs.get('safe'):
            ...
            if safe not in (0, 1):
                return Status(
                    404, StatusEnum.FAILURE.value, '[企业微信]safe参数值不合法，只允许0或者1', {}).json()
        ...
        if kwargs.get('enable_id_trans'):
            ...
            if enable_id_trans not in (0, 1):
                return Status(
                    404, StatusEnum.FAILURE.value, '[企业微信]enable_id_trans参数值不合法，只允许0或者1', {}).json()
        ...
        if kwargs.get('enable_duplicate_check'):
            ...
            if enable_duplicate_check not in (0, 1):
                return Status(
                    404, StatusEnum.FAILURE.value, '[企业微信]enable_duplicate_check参数值不合法，只允许0或者1', {}).json()
        ...
        if kwargs.get('duplicate_check_interval'):
            ...
            if duplicate_check_interval < 1 or duplicate_check_interval > 4 * 60 * 60:
                return Status(
                    404, StatusEnum.FAILURE.value, '[企业微信]duplicate_check_interval参数值不合法，只允许1秒至4小时之间', {}).json()
        # 参数 >>>>> 基础data对象参数
        format_content = self.format_content(ftype=stype, content=content)
        if not format_content:
            return Status(
                404, StatusEnum.FAILURE.value, '[企业微信]格式化的消息体为空', {}).json()
        # 发送的数据
        data = {
        ...
        try:
            ...
            if not status:
                return Status(
                    902, StatusEnum.FAILURE.value, response.get('errmsg'), response).json()
            if response.get("errcode") == 0 and response.get("errmsg") == "ok":
                return Status(
                    100, StatusEnum.SUCCESS.value, StatusMsgs.get(100), response).json()
            else:
                return Status(
                    902, StatusEnum.FAILURE.value, response.get('errmsg'), response).json()
        except Exception as error:
            LOG.error("[QYWXLib]send message occur exception：%s" % error)
            return Status(902, StatusEnum.FAILURE.value, "[企业微信]发送消息异常，请稍后重试", {}).json()
    ...
    def sendback(self, message_id: str) -> json:
        ...
        if not message_id:
            return Status(
                400, StatusEnum.FAILURE.value, '[企业微信]缺少message_id参数', {}).json()
        if not isinstance(message_id, str):
            return Status(
                402, StatusEnum.FAILURE.value, '[企业微信]message_id参数支持str类型', {}).json()
        ...
        try:
            ...
            return Status(902, StatusEnum.FAILURE.value, "[企业微信]撤回消息异常，请稍后重试", {}).json()
    ...
    def temp_upload(self, upload_type: str, upload_name: str, upload_file: str) -> json:
        ...
        if not upload_type:
            return Status(
                400, StatusEnum.FAILURE.value, '[企业微信]缺少upload_type参数', {}).json()
        if not isinstance(upload_type, str):
            return Status(
                402, StatusEnum.FAILURE.value, '[企业微信]upload_type参数支持str类型', {}).json()
        if upload_type not in self.temp_upload_types:
            return Status(
                404, StatusEnum.FAILURE.value, '[企业微信]upload_type参数不支持请求类型', {}).json()
        if not upload_file \
                or not os.path.exists(upload_file) \
                or not os.path.isfile(upload_file):
            return Status(
                450, StatusEnum.FAILURE.value, '[企业微信]缺少上传文件', {}).json()
        ...
        if not verify_status:
            return Status(
                454, StatusEnum.FAILURE.value, verify_message, {}).json()
        ...
        try:
            ...
            return Status(902, StatusEnum.FAILURE.value, "[企业微信]文件上传异常，请稍后重试", {}).json()
